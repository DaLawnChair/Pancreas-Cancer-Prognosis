...
# ! jupyter nbconvert --to python twoClassClassification.ipynb --output test27-8.py
...
def generateKFoldsValidation(testInformation, dataset, dataframes, k=5, trainingTransform=None):
#     testInformation = {
#     'testName' : testName,
#     'batchSize': batchSize,
...
def getDataframes():
    dataframeLoad = pd.read_excel(dataframePath, sheet_name=None,header=None, names=columns)
    dataframes = {
...
def gridSearch(testInformation):
    ...
    generateKFoldsValidation(testInformation, dataset, dataframes, k=5, trainingTransform=None) 
...
def setGridSearchParams():
    ...
    for segmentsMultiple in [3,6,9,12]:
                ...
                for weight_decay in [0.01,0.001]:
                    for batchSize in [16,32]:
                        for patience in [5,10]:
                            testName = f'testSamples27-8/gridSearch{modelChosen}-segmentsMultiple={segmentsMultiple}'
