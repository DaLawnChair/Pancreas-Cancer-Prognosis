...
# ! jupyter nbconvert --to python imageSegmentation2Classes.ipynb --output testSamples32-7.py
...
def evaluateModelOnTestSet(testPathName, model, testingData, criterion, device, saveConfusionMatrix = True, showConfusionMatrix=True):
    ...
    for i in range(len(originalLabelsTemp)):  
        if i%segmentsMultiple==0:
            originalLabels.append(np.array([originalLabelsTemp[i] >=0.5]))  
        i+=1
    ans, ansTotal = convertTensorToPredictions( originalLabels, {0:0,1:0})
