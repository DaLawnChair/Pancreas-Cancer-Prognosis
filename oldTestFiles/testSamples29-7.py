...
# ! jupyter nbconvert --to python imageSegmentationWithBackground.ipynb --output testSamples29-7.py
...
def evaluateGroupVoting(model, loader, criterion, device):
    ...
    ## Majority voting for finding the classification of the patient
    for i in range(0, len(all_probs), segmentsMultiple):
        ...
        patient_labels = all_labels[i:i + segmentsMultiple]
        ...
        _, label_counts = np.unique(patient_labels, return_counts=True)
        ...
        for idx in range(len(label_counts)):
            ...
            ## For average
            # patient_pred = np.mean(all_preds[i:i + segmentsMultiple])
            # patient_label = np.mean(all_labels[i:i + segmentsMultiple])
            patient_probs.append(patient_prob[maxIdx])
            patient_labels.append(maxIdx)

    patient_probs = torch.from_numpy(np.array(patient_probs))
    patient_labels = torch.from_numpy(np.array(patient_labels))

    return epoch_loss, epoch_acc, patient_labels
...
def trainModel(model, criterion, optimizer, trainingData, validationData, numOfEpochs=100):
    ...
    for epoch in range(numOfEpochs):
        ...
        curValLoss, curValAcc, _ = evaluateGroupVoting(model, validationData, criterion, device)    
...
def evaluateModelOnTestSet(testPathName, model, testingData, criterion, device, saveConfusionMatrix = True, showConfusionMatrix=True):
    test_loss, test_acc, predictions = evaluateGroupVoting(model, testingData, criterion, device)
