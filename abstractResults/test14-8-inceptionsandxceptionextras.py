...
def setGridSearchParams():
    ...
    for segmentsMultiple in [12]:
        for modelChosen in ['InceptionV3Small2D', 'XceptionSmall2D']: #, 'ResNet50Small2D','InceptionV3Small2D','VGG16Small2D', 'XceptionSmall2D']:
            for learningRate in [0.001,0.0001]:
                for weight_decay in [0.01,0.001]:
                    for batchSize in [16,32]:
                        for patience in [5,10]:
                            testName = f'testSamples14-8/gridSearch{modelChosen}-segmentsMultiple={segmentsMultiple}'
