...
# }
    ...
    # Get the testing set
    testIndicies = getTestingSet(dataset).keys()
    # Remove the testing set from the dataset
    for testCase in testIndicies:
        del dataset[testCase]

    patients = list(dataset.keys())
    fakeData = [-1] * len(patients)

    # Get rid of the patients dataset that are in the testing set
    dataForSplits = list(set(patients).symmetric_difference(set(testIndicies)))
